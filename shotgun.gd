...
func _physics_process(_delta):
	...
	match state:
		PICKEDUP:
			position = position.move_toward(Vector2(0,-4),.4)
			z_index = 0
...
func shoot():
	position = position + Vector2(1,0).rotated(rotation) * -4
	for i in randi_range(15,20):
		var bullet_speed = randi_range(150,250)
		var ranDir = randi_range(15,-15)
		...
		bullet.linear_damp = (100/log(bullet_speed))-17
